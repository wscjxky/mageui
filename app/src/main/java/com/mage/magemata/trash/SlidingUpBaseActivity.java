...
public abstract class SlidingUpBaseActivity<S extends Scrollable> extends BasecActivity implements ObservableScrollViewCallbacks {
    ...
    private ShineButton button_heart;
        ...
        ScrollUtils.addOnGlobalLayoutListener(mInterceptionLayout, new Runnable() {
            ...
            public void run() {
                if (button_heart != null) {
                    ViewHelper.setTranslationX(button_heart, mTitle.getWidth() - mFabMargin - button_heart.getWidth());
                    ViewHelper.setTranslationY(button_heart, ViewHelper.getX(mTitle) - (button_heart.getHeight() / 2));
                }
    ...
    private TouchInterceptionFrameLayout.TouchInterceptionListener mInterceptionListener = new TouchInterceptionFrameLayout.TouchInterceptionListener() {
        ...
        public boolean shouldInterceptTouchEvent(MotionEvent ev, boolean moving, float diffX, float diffY) {
            ...
            button_heart.getHitRect(fabRect);
...
//            mHeader.setVisibility(View.VISIBLE);
    ...
    private void showFab(boolean animated) {
        ...
        if (!mFabIsShown) {
            if (animated) {
                ViewPropertyAnimator.animate(button_heart).cancel();
                ViewPropertyAnimator.animate(button_heart).scaleX(1).scaleY(1).setDuration(200).start();
            } else {
                ViewHelper.setScaleX(button_heart, 1);
                ViewHelper.setScaleY(button_heart, 1);
            }
            mFabIsShown = true;
        } else {
    ...
    private void hideFab(boolean animated) {
        ...
        if (mFabIsShown) {
            if (animated) {
                ViewPropertyAnimator.animate(button_heart).cancel();
                ViewPropertyAnimator.animate(button_heart).scaleX(0).scaleY(0).setDuration(200).start();
            } else {
                ViewHelper.setScaleX(button_heart, 0);
                ViewHelper.setScaleY(button_heart, 0);
            }
            mFabIsShown = false;
        } else {
